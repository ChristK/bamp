...
#' @param rho AR1 coefficient for \code{deviation = "ar1"} (\code{0 <= rho < 1}); the coherence
#'   dial -- larger \code{rho} allows the sex gap to persist longer before reverting. Ignored for
#'   \code{"iid"}.
#' @param mcmc list with \code{iterations}, \code{burn_in}, \code{thin}.
#' @param hyper list of Gamma hyperparameters \code{c(a, b)} for the precisions
#'   \code{age}, \code{period}, \code{cohort} and \code{dev} (the deviation precision).
...
bamp_coherent <- function(cases, population, age = "rw1", period = "rw1", cohort = "rw1",
  ...
  for (it in seq_len(iters)) {
    ...
    if (it %in% keep) {
      ...
      out$lambda_d[store] <- ld; out$kappa_theta[store] <- kth
    }
  }
  ...
  structure(list(
    ...
    model = list(age = age, period = period, cohort = cohort,
                 deviation = deviation, rho = rho, ord = c(ord_a, ord_p, ord_c)),
...
predict_coherent <- function(object, periods = 0, population = NULL,
  ...
  for (d in seq_len(D)) {
    ...
    de <- .cj_predict_ar(c(s$delta[d, ], numeric(n2 - n1)), s$lambda_d[d], rho, n1, n2)
    dev_ext[, d] <- de
    for (i in 1:I) {
      kk <- (I - i) * M + (1:n2)
      base <- s$mu0[d] + ph + ps[kk]
      etaF <- base + s$a[d, 1] + s$theta[d, 1, i] + sgn[1] * de
      etaM <- base + s$a[d, 2] + s$theta[d, 2, i] + sgn[2] * de
